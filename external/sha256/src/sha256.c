...
static inline void consume_chunk(uint32_t *h, const uint8_t *p) {
    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 16; j++) {
            ...
            const uint32_t s1 = right_rot(ah[4], 6) ^ right_rot(ah[4], 11) ^
                                right_rot(ah[4], 25);
            ...
            const uint32_t s0 = right_rot(ah[0], 2) ^ right_rot(ah[0], 13) ^
                                right_rot(ah[0], 22);
            const uint32_t maj =
...
uint8_t *sha_256_close(struct Sha_256 *sha_256) {
    ...
    /*
     * The current chunk cannot be full. Otherwise, it would already have be
    ...
    *pos++ = 0x80;
     ...
     * chunk, and zero padding before that. But we do not necessarily have
     * enough space left. If not, we pad the current chunk with zeroes, and add
